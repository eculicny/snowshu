...
DOCKER_SPIN_UP_TIMEOUT = 20

@pytest.fixture(scope="session", autouse=True)
def end_to_end(docker_flush_session):
    client=docker.from_env()
    last_test = client.containers.get('integration-test')
    last_test.remove(force=True)

    runner = CliRunner()
    configuration_path = os.path.join(
        PACKAGE_ROOT, 'snowshu', 'templates', 'replica.yml')
    create_result=runner.invoke(cli, ('create', '--replica-file', configuration_path, '--compile-method', 'updated'))
