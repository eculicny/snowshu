...
# TODO fix unbound relation vars
# TODO better parameterize query strings (i.e. cols)
...
def test_analyze_unsampled(stub_relation_set):
    ...
    upstream = RuntimeSourceCompiler.compile_queries_for_relation(upstream, dag, adapter, True, "updated")
...
def test_run_unsampled(stub_relation_set):
    ...
    dag.add_edges_from([(upstream, stub_relation_set.downstream_relation,)])
    adapter = SnowflakeAdapter()
    upstream = RuntimeSourceCompiler.compile_queries_for_relation(upstream, dag, adapter, False, "updated")
    assert query_equalize(upstream.compiled_query) == query_equalize(f"""
        SELECT
            *
...
def test_analyze_iso(stub_relation_set):
    ...
    adapter = SnowflakeAdapter()
    iso = RuntimeSourceCompiler.compile_queries_for_relation(iso, dag, adapter, True, "updated")
...
def test_run_iso(stub_relation_set):
    ...
    dag.add_nodes_from([iso])
    adapter = SnowflakeAdapter()
    iso = RuntimeSourceCompiler.compile_queries_for_relation(iso, dag, adapter, False, "updated")
    assert query_equalize(iso.compiled_query) == query_equalize(f"""
        SELECT
            *
        FROM 
...
def test_run_deps_directional(stub_relation_set):
    ...
    adapter=SnowflakeAdapter()
    upstream = RuntimeSourceCompiler.compile_queries_for_relation(upstream,dag,adapter,False, "updated")
    downstream = RuntimeSourceCompiler.compile_queries_for_relation(downstream,dag,adapter,False, "updated")
...
def test_run_deps_bidirectional_include_outliers(stub_relation_set):
    ...
    adapter=SnowflakeAdapter()
    RuntimeSourceCompiler.compile_queries_for_relation(upstream,dag,adapter,False, "updated")
    RuntimeSourceCompiler.compile_queries_for_relation(downstream,dag,adapter,False, "updated")
    assert query_equalize(downstream.compiled_query)==query_equalize(f"""
        WITH {downstream.scoped_cte('SNOWSHU_FINAL_SAMPLE')} AS (
            SELECT
                *
        ...
        ,{downstream.scoped_cte('SNOWSHU_DIRECTIONAL_SAMPLE')} AS (
            SELECT
                *
            FROM {downstream.scoped_cte('SNOWSHU_FINAL_SAMPLE')}
            SAMPLE BERNOULLI (1500 ROWS)
        )
        SELECT
            *
        FROM {downstream.scoped_cte('SNOWSHU_DIRECTIONAL_SAMPLE')}
        UNION (SELECT
                *
            FROM {downstream.quoted_dot_notation}
...
def test_run_deps_directional_line_graph():
    ...
    adapter=SnowflakeAdapter()
    RuntimeSourceCompiler.compile_queries_for_relation(relation_a, dag, adapter, False, "updated")
    RuntimeSourceCompiler.compile_queries_for_relation(relation_b, dag, adapter, False, "updated")
    RuntimeSourceCompiler.compile_queries_for_relation(relation_c, dag, adapter, False, "updated")
...
def test_run_deps_bidirectional_line_graph():
    ...
    """)
    assert query_equalize(relation_c.compiled_query) == query_equalize(f"""
        WITH {relation_c.scoped_cte('SNOWSHU_FINAL_SAMPLE')} AS (
            SELECT
                *
        ...
        ,{relation_c.scoped_cte('SNOWSHU_DIRECTIONAL_SAMPLE')} AS (
            SELECT
                *
            FROM {relation_c.scoped_cte('SNOWSHU_FINAL_SAMPLE')}
            SAMPLE BERNOULLI (1500 ROWS) )
        SELECT
            *
        FROM {relation_c.scoped_cte('SNOWSHU_DIRECTIONAL_SAMPLE')}
    """)
...
def test_run_deps_mixed_multi_deps():
    ...
    RuntimeSourceCompiler.compile_queries_for_relation(relation_b, dag, adapter, False, "updated")
    RuntimeSourceCompiler.compile_queries_for_relation(relation_c, dag, adapter, False, "updated")
    RuntimeSourceCompiler.compile_queries_for_relation(relation_d, dag, adapter, False, "updated")
    RuntimeSourceCompiler.compile_queries_for_relation(relation_e, dag, adapter, False, "updated")
    assert query_equalize(relation_a.compiled_query) == query_equalize(f"""
        WITH {relation_a.scoped_cte('SNOWSHU_FINAL_SAMPLE')} AS ( 
        SELECT 
            * 
    ...
    assert query_equalize(relation_d.compiled_query) == query_equalize(f"""
        WITH 
        {relation_d.scoped_cte('SNOWSHU_FINAL_SAMPLE')} AS ( 
        SELECT 
            * 
        FROM 
        {relation_d.quoted_dot_notation}
        ...
        ,{relation_d.scoped_cte('SNOWSHU_DIRECTIONAL_SAMPLE')} AS ( 
        SELECT 
            * 
        FROM 
        {relation_d.scoped_cte('SNOWSHU_FINAL_SAMPLE')} SAMPLE BERNOULLI (1500 ROWS) 
        ) 
        SELECT 
            * 
        FROM 
        {relation_d.scoped_cte('SNOWSHU_DIRECTIONAL_SAMPLE')}
    """)
